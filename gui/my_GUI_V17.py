...
class MainWindow(QMainWindow):
    ...
    def getOuputs(self):
        self.wavelossAttentionOutput=utils.generate_prediction(self.directory, model_path)
        self.wavelossOutput=utils.generate_prediction(self.directory, model_path)
        self.baselineAttentionOutput=utils.generate_prediction(self.directory, model_path)
        self.baselineOutput=utils.generate_prediction(self.directory, model_path)
    
    def updateViews(self):
        axis = self.combobox_axis.currentIndex()
        layer = self.combobox_layer.currentIndex()
        self.wavelossAttention_loaded_plot.display(self.wavelossAttentionOutput, axis, layer)
        self.baselineAttention_loaded_plot.display(self.baselineAttentionOutput, axis, layer)
        self.waveloss_loaded_plot.display(self.wavelossOutput, axis, layer)
        self.baseline_loaded_plot.display (self.baselineOutput, axis, layer)
